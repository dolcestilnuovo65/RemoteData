...
/// <reference path="../../lib/backbone-global.d.ts" />
/// <reference path="../../contracts/IRemoteData.ts" />
/// <reference path="../../remoteData/remoteData/DBFS.ts" />
...
interface String {
    charAtIsUpper(atpos: number): boolean;
}
...
class RemoteData extends Backbone.Model implements IRemoteData {
...
//    public isContained: boolean = false; // nel caso sia un array non fa creare la directory contenitore
    ...
    public remoteDataFactory: IRemoteDataFactory = null;
    ...
    public parse2(data) {
             let ISO_8601_FULL: RegExp = /^\d{4}-\d\d-\d\dT\d\d:\d\d:\d\d(\.\d+)?(([+-]\d\d:\d\d)|Z)?$/i
             let all = Object.getOwnPropertyNames(this);
             for (var elem in all) {
                 if (_.isString(data[all[elem]]) && ISO_8601_FULL.test(data[all[elem]]))
                     data[all[elem]] = new Date(data[all[elem]])
             }   
             return data;
...
class RemoteServer extends Backbone.Model {
    ...
    public getDirsList(database: string, instance: string, virtualPath: string, page: number, itemsPerPage: number, orderBy: string, filter?: string, dummy?: any): string[] {
    ...
    public getDataBlock(process:string, filter: string): string {
        //static finally command = "";
    ...
    public getData(data: Object, action :string): string {
    ...
    public getFoto(key: string, database: string, instance: string): string[] {
        //static finally command = "";
